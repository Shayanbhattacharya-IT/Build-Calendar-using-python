...
<Br><Br>
	2 (width): The width of the date columns in characters (default is 2).<Br><Br>
	1 (lines per week): The number of lines used for each week (default is 1).<Br><Br>
	8 (column separation): The number of spaces between month columns (default is 6).<Br><Br>
	3 (months per row): The number of months to display per row in the year calendar layout (default is 3).<Br><Br>
